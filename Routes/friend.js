...
router.post("/request", [authenticator], async (req, res) => {
	...
	try {
		if (req.user.role == "mentor")
			return res.status(401).send({ res: "Mentor cannot send requests" });
...
router.put("/request/:requestId", [authenticator], async (req, res) => {
	try {
		...
		if (role == "student" && req.body.status == "accepted")
			return res
				.status(401)
				.send({ res: "Requestor cannot accept the request" });
		const query = { _id: req.params.requestId };
		const friend = await Friend.findOne(query);
		if (!friend)
			return res.status(400).send({ res: "Friend request doesn't exists." });
		...
		if (friend.status == "accepted") friend.messages.push(firstmessage);
		await friend.save();
...
router.get("/chat", [authenticator], async (req, res) => {
	...
	try {
		if (role == "student")
			query = { studentId: req.user._id, status: "accepted" };
...
router.post("/chat/:chatId", [authenticator], async (req, res) => {
	...
	try {
		...
		if (chat.status !== "accepted")
			return res.status(400).send({ res: "Friend request not accepted." });
		const message = {
			text: req.body.text,
			sender: req.user._id,
			time: new Date(),
...
router.get("/getChatId/:mentorId", [sAuthenticator], async (req, res) => {
	...
	try {
		...
		//console.log(chat);
		if (!chat)
			return res.status(400).send({ res: "Friend request not sent/accepted" });
...
router.get("/request", [authenticator], async (req, res) => {
	...
	try {
		if (role == "student")
			query = {
				$and: [{ studentId: req.user._id }, { status: { $ne: "accepted" } }],
			};
		else
			query = {
				$and: [{ mentorId: req.user._id }, { status: { $ne: "accepted" } }],
			};
...
router.get("/", [authenticator], async (req, res) => {
	...
	else filter = { studentId: req.user._id, status: "accepted" };
	try {
